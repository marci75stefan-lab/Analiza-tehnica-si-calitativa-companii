...
# Inlined rather than imported from a sibling module: Vercel's Python
# serverless runtime bundles each api/*.py file as an independent function
# and a plain `import i18n` (which works fine when running this file
# directly, since Python adds the script's own directory to sys.path) was
# not reliably resolving there, causing every /api/analyze request to fail
# with a 500 in production.
